...
public class MorphologicalWordAnalysis : ValueObject
{
    ...
    private MorphologicalWordAnalysis(string word, string lexeme, TagSet tagSet, double probability)
    {
    ...
    public static MorphologicalWordAnalysis Create(string raw)
    {
        ...
        return new MorphologicalWordAnalysis(parts[0], parts[1], TagSet.FromRaw(parts[2]), double.Parse(parts[3]));
    }
    ...
    public override string ToString() => $"{Word} {Lexeme} {TagSet.PartOfSpeech} {Probability}";
}